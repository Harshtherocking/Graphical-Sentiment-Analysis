...
class GCN (MessagePassing):
    def __init__ (self, in_channel: int, out_channel: int) : 
        ...
        self.dep_Embed = Embedding(num_embeddings = num_embeddings, embedding_dim= out_channel, scale_grad_by_freq= True)
    ...
    def forward(self, G : Data) : 
        ...
        x = self.W1(x)
        edge_attr = self.dep_Embed(edge_attr)
...
class GcnDenseModel (Module):
    def __init__(self, input_feature_size : int, output_feature_size : int):
        ...
        self.gcn = GCN(input_feature_size, output_feature_size)
